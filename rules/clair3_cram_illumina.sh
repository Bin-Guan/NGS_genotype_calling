...
#lscratch needs to be 3xcram file size + 100G for clair3; cram: 49G, bam: 117G, total required: 235G; Time: 5.5h; mem=70G
set -e
...
echo "lscratch use after cram2bam"
du -sh $LWORK_DIR
...
echo "lscratch use after clair3"
du -sh $LWORK_DIR
...
cp $LWORK_DIR/merge_output.vcf.gz clair3/vcf/$sample.vcf.gz
cp $LWORK_DIR/merge_output.vcf.gz.tbi clair3/vcf/$sample.vcf.gz.tbi
