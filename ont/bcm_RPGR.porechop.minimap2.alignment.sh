...
module load porechop minimap2/2.26 sambamba/0.8.2 mosdepth whatshap/2.3
...
then
	...
	for fastq in fastq/*.fastq.gz; do
		...
		porechop --discard_unassigned  --discard_middle --extra_end_trim 0 -i $fastq -b porechop/$sample
	done
fi
