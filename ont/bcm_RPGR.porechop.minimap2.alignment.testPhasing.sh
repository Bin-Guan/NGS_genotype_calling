...
then
	...
	while read -r sample;
	do
		porechop --discard_unassigned  --discard_middle --extra_end_trim 0 -i fastq/$sample.fastq.gz -b porechop/$sample
	done < manifest.csv
	export PORECHOP_ADAPTERS=/data/OGL/resources/porechop/adaptersBCM.py
	while read -r sample;
	do
	porechop --discard_unassigned  --discard_middle --extra_end_trim 0 -i porechop/$sample/BCE6R.fastq.gz -b porechop/$sample/bcm
	porechop --discard_unassigned  --discard_middle --extra_end_trim 0 -i porechop/$sample/BCRPGRr.fastq.gz -b porechop/$sample/RPGR
...
echo $coverage
if (( $coverage > 5 )); then
#freebayes for haplotype-tagging of MW
freebayes -f /data/OGL/resources/genomes/NCBI/GRCh38Decoy/genome.fa -p 6 -C 5 -F 0.1 \
  ...
  --region chrX:154180596-154198861 \
  bam/$sample.bam \
  | vcffilter -f "QUAL > 10" \
...
tabix -f -p vcf clair3/$sample.norm.vcf.gz
...
while read -r sample; do
bcftools concat --threads 1 -a --no-version \
deepvariant/$sample.RPGR.dv.phased.vcf.gz deepvariant/$sample.OPN1.dv.phased.vcf.gz -Oz \
-o deepvariant/$sample.dv.phased.vcf.gz
tabix -f -p vcf deepvariant/$sample.dv.phased.vcf.gz
whatshap haplotag --output-threads 3 -o bam/$sample.RPGR-LW.haplotag.bam --reference /data/OGL/resources/genomes/NCBI/GRCh38Decoy/genome.fa \
  --ignore-read-groups --skip-missing-contigs deepvariant/$sample.dv.phased.vcf.gz bam/$sample.RPGR-LW.bam
samtools index -@ 3 bam/$sample.RPGR-LW.haplotag.bam
whatshap haplotag --output-threads 3 --ploidy 6 -o bam/$sample.MW.haplotag.bam --reference /data/OGL/resources/genomes/NCBI/GRCh38Decoy/genome.fa \
  --ignore-read-groups --skip-missing-contigs freebayes/$sample.MW.phased.vcf.gz bam/$sample.MW.bam
samtools index -@ 3 bam/$sample.MW.haplotag.bam
sambamba merge -t 3 -l 6 bam/$sample.haplotag.bam bam/$sample.RPGR-LW.haplotag.bam bam/$sample.MW.haplotag.bam
done < manifest.csv
...
#Next step: remove more intermediate files
