...
module load chopper/0.7.0 porechop minimap2/2.26 sambamba/0.8.2

mkdir -p bam fastqNoCommonAdapter filteredFastq porechop
case "${library^^}" in
	"QBPLEX")
...
then
	...
	for fastq in fastq/*.fastq.gz; do
		...
		porechop --discard_unassigned -i $fastq -b porechop/$sample
	done
fi
...
while read -r sample;
do
porechop --discard_unassigned -i porechop/$sample/BCE6R.fastq.gz -b porechop/$sample
minimap2 -a -x map-ont -Y -t 3 -R "@RG\tLB:$library\tID:$sample\tSM:$sample\tPL:ONT" /data/OGL/resources/genomes/GRCh38/OPN1LW/GRCh38Decoy_OPN1LW.mmi porechop/$sample/BCLW.fastq.gz \
...
minimap2 -a -x map-ont -Y -t 3 -R "@RG\tLB:$library\tID:$sample\tSM:$sample\tPL:ONT" /data/OGL/resources/genomes/GRCh38/OPN1MW/GRCh38Decoy_OPN1MW.mmi porechop/$sample/BCMW.fastq.gz \
...
sambamba merge -t 3 -l 6 bam/$sample.bam bam/$sample.LW.bam bam/$sample.MW.bam
rm bam/$sample.LW.bam* bam/$sample.MW.bam*
done < manifest.csv
...
module load freebayes/1.3.5 deepvariant/1.6.0 R/4.3.0 whatshap/2.3 annovar/2020-06-08
