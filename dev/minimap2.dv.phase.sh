...
echo "manifest.csv present"
else 
for fastq in fastq/*.fastq.gz; do
sample=$(basename $fastq | cut -d "_" -f 1);
echo -e "$sample" >> manifest.csv
minimap2 -a -x map-ont -Y -t 3 -R "@RG\tLB:QBplEx\tID:$sample\tSM:$sample\tPL:ONT" /data/OGL/resources/genomes/GRCh38/OPN1LW/GRCh38Decoy_OPN1LW.mmi $fastq | sambamba sort -u --compression-level 6 --tmpdir=/lscratch/$SLURM_JOB_ID -t 3 -o bam/$sample.bam <(sambamba view -S -f bam --compression-level 0 -t 3 /dev/stdin);
done
fi
...
module load deepvariant/1.6.0 R/4.3.0 whatshap/1.1 annovar/2020-06-08
mkdir -p deepvariant
mkdir -p annotation
...
| bcftools annotate --set-id 'dv_%CHROM\:%POS%REF\>%ALT' -x ^FORMAT/GT,FORMAT/DP,FORMAT/VAF --output-type u --no-version \
